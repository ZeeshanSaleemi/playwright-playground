...
test.describe('Homepage', () => {
  ...
  test('article cards have headline and image', async () => {
  ...
  test('clicking first article card opens article page', async ({ page }) => {
  ...
  test('clicking sports nav link opens sports page', async ({ page }) => {
    await homePage.clickNavLink('Sports');
    await expect(page).toHaveURL(/sport/);
  });

  test('first article card has a valid link', async () => {
    await expect(homePage.articleCards.first()).toHaveAttribute('href', /article/);
  });

  test('navigation bar has enough links', async () => {
